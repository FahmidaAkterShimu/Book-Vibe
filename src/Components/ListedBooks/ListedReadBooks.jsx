...
const ListedReadBooks = ({ sortingType }) => {
    ...
    const [filteredReadBooks, setFilteredReadBooks] = useState(readBooks)
    ...
    useEffect(() => {
        if (sortingType) {
            if (sortingType === 'pages') {
                ...
                setFilteredReadBooks(sortedData)
            }
    ...
    if (filteredReadBooks.length === 0) {
        return <div className='h-[50vh] flex justify-center items-center bg-gray-100 rounded-2xl mt-8'>
            <h2 className='text-3xl font-bold'>There are no read books found.</h2>
        </div>
    }
    ...
    return (
        <div className='mt-8'>
            <div className='grid md:grid-cols-3 gap-6'>
                {
                    filteredReadBooks.map((book, ind) => (
